...
5. **5-Category Major Subject Drift Detection & Quality Rater Feedback Loop**
   - **Technical Implementation:** `QualityRaterAgent` executes an LLM pre-processing step (`consolidate_rubric_with_llm`) to synthesize scene goals and drift parameters, returning Pydantic `QualityEvaluationResult` with overall `score` and per-criterion `reason` breakdown.
