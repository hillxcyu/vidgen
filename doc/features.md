...
9. **Showcase Run Pinning & GCS Cloud Storage Sync**
   - **Technical Implementation:** Pinned runs persist stitched MP4s, shot clips, frame PNGs, and JSON manifests to GCS (`gs://...`), prioritizing public GCS HTTPS URLs with fallback to local `/output/` static paths.
