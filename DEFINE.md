...
- [ ] `[T001]` **[agent]** Import `AgentTool` in `app/agent.py`.
- [ ] `[T002]` **[agent]** Refactor `ScreenwriterAgent`, `StoryboarderAgent`, `PromptOptimizerAgent`, `HealthCheckerAgent`, and `QualityRaterAgent` instructions for clean tool-return execution.
- [ ] `[T003]` **[agent]** Wrap the 5 sub-agents in `AgentTool(...)` and register in `root_agent.tools`, setting `sub_agents=[]` on `root_agent`.

### Phase 2: Testing & CI/CD [test] [deploy]
- [ ] `[T004]` **[test]** Update `tests/unit/test_agent.py` to assert `AgentTool` names in `root_agent.tools`.
...
- [ ] `[T008]` **[verify]** Verify live reasoning engine deployment and context cache alignment.
